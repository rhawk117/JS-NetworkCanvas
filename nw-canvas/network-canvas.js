 ...
 * @type {Config}
 */
const getNodeCountForScreen = () => {
	const width = window.innerWidth || 1200;
	const height = window.innerHeight || 800;
	const isMobile = width <= 600 || height <= 600;
	if (isMobile) {
		return Math.max(10, Math.min(40, Math.floor((width * height) / 20000)));
	}
	// Increase minimum and maximum for PC
	return Math.max(100, Math.min(800, Math.floor((width * height) / 4000)));
};

const config = {
	nodeCount: getNodeCountForScreen(),
	frequency: 500,
	justifyNodes: "random",
};
